...
from subspace_acquicisiton_functions import SubspaceProbabilityOfImprovement,SubspaceqProbabilityOfImprovement
import warnings
...
print(f'SSPI on {num_vari}-D {fun_name}, batch size: {batch_size}, iterations: {iteration}, evaluations: {evaluation}, fmin: {-fmin.item()}')
while evaluation < max_evaluation:
    ...
    for i in range(num_subspaces): 
        real_q = q_list[i]            
        if real_q == 1:
            acq_fun = SubspaceProbabilityOfImprovement(model=model,best_f=fmin,best_x=xmin,subspace=subspaces[i])
        else:
            acq_fun = SubspaceqProbabilityOfImprovement(model=model,
                                                        best_f=fmin,
                                                        best_x=xmin,
