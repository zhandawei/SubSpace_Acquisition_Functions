...
from subspace_acquicisiton_functions import SubspaceUpperConfidenceBound,SubspaceqUpperConfidenceBound
import warnings
...
print(f'SSUCB on {num_vari}-D {fun_name}, batch size: {batch_size}, iterations: {iteration}, evaluations: {evaluation}, fmin: {-fmin.item()}')
while evaluation < max_evaluation:
    ...
    for i in range(num_subspaces): 
        real_q = q_list[i]            
        if real_q == 1:
            acq_fun = SubspaceUpperConfidenceBound(model=model,beta=0.1,best_x=xmin,subspace=subspaces[i])
        else:
            acq_fun = SubspaceqUpperConfidenceBound(model=model,
                                                        beta=0.1,
                                                        best_x=xmin,
